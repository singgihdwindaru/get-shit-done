...
Check code says:  hooksDir = path.join(configDir, 'hooks')
                  configDir = ~/.claude
                  → checks ~/.copilot/hooks/

Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = ~/.copilot/get-shit-done
                  → writes to ~/.copilot/get-shit-done/hooks/
...
```bash
INIT=$(node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
