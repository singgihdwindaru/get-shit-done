...
- **Global scope**: `~/.copilot/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `~/.copilot/notes/`)
...
2. Glob `~/.copilot/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `~/.copilot/notes/` — works in any directory
