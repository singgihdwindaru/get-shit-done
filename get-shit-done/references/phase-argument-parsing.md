...
```bash
PHASE_INFO=$(node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.copilot/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
